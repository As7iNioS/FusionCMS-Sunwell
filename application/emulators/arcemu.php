...
class Arcemu implements IEmulator
{
	...
	private $queries = array(
		...
		"get_banned" => "SELECT acct id, banreason banreason, banned banned FROM accounts WHERE acct=? AND banned=1",
		...
		"get_inventory_item" => "SELECT slot slot, guid item, entry itemEntry FROM playeritems WHERE slot >= 0 AND slot <= 18 AND ownerguid=?",
	...
	public function sendItems($character, $subject, $body, $items)
	{
		...
		foreach($items as $item)
		{
			...
			if(!isset($queries[$mail_id]))
			{
				$queries[$mail_id] = array(
					"stationary" => "61",
					"sender_guid" => 0,
			...
			if($item_count == 1)
			{
				$queries[$mail_id]["item_id"] = $item['id'];
				$queries[$mail_id]["item_stack"] = 1;
			}
			else
			{
				$queries[$mail_id]["item_id".$item_count] = $item['id'];
				$queries[$mail_id]["item_stack".$item_count] = 1;
			}
		}
		
		foreach($queries as $query)
		{
			get_instance()->realms->getRealm($this->config['id'])->getCharacters()->getConnection()->insert("mailbox_insert_queue", $query);
		}
	}
