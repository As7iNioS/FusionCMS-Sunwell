...
class Trinity_tbc_soap implements IEmulator
{
	...
	protected $queries = array(
		...
		"pvp_character" => "SELECT *, CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(`data`, ' ', 1514), ' ', -1) AS UNSIGNED) AS `totalKills` FROM characters WHERE CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(`data`, ' ', 1514), ' ', -1) AS UNSIGNED) > 0 ORDER BY CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(`data`, ' ', 1514), ' ', -1) AS UNSIGNED) DESC LIMIT ",
		"get_character" => "SELECT *, CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(`data`, ' ', 1454), ' ', -1) AS UNSIGNED) AS `totalKills` FROM characters WHERE guid=?",
